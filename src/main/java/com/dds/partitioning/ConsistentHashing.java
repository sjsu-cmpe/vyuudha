/**
 * ConsistentHashing is taken from lib_ketama: https://github.com/RJ/ketama/tree/master/java_ketama
 * 
 */
 ...
 *
 *		ConsistentHashing pool = ConsistentHashing.getInstance();
 ...
 *		pool.setHashingAlg( ConsistentHashing.NEW_COMPAT_HASH );	
 *		pool.initialize();	
 *	}
 *  </pre> 
 ...
 * <pre>
 *		ConsistentHashing.SockIO sock = ConsistentHashing.getInstance().getSock( key );
 *		try {
 *			sock.write( "version\r\n" );	
 *			sock.flush();	
...
public class ConsistentHashing {
	...
	private static Logger log =
		Logger.getLogger( ConsistentHashing.class.getName() );
	...
	// empty constructor
	protected ConsistentHashing() { }
	
	/** 
	 * Factory to create/retrieve new pools given a unique poolName. 
	 * 
	 * @param poolName unique name of the pool
	 * @return instance of ConsistentHashing
	 */
	@SuppressWarnings("unchecked")
	public static synchronized ConsistentHashing getInstance( String poolName ) {
		if ( pools.containsKey( poolName ) )
			return (ConsistentHashing)pools.get( poolName );

		ConsistentHashing pool = new ConsistentHashing();
	 ...
	 * @return instance of ConsistentHashing
	 */
	public static synchronized ConsistentHashing getInstance() {
	 ...
	 *
	 * ConsistentHashing.NATIVE_HASH (0)     - native String.hashCode() - fast (cached) but not compatible with other clients
	 * ConsistentHashing.OLD_COMPAT_HASH (1) - original compatibility hashing alg (works with other clients)
	 * ConsistentHashing.NEW_COMPAT_HASH (2) - new CRC32 based compatibility hashing algorithm (fast and works with other clients)
	 * 
	...
	public SockIO getSock( String key, Integer hashCode ) {
		...
		while ( tries++ < bucketSize ) {
			...
			String server = buckets.get(ConsistentHashing.findPointFor(hv));
	...
	public synchronized void shutDown() {
		log.warn( "++++ ConsistentHashing shutting down..." );
		...
		hostDead    = null;
		initialized = false;
		log.warn( "++++ ConsistentHashing finished shutting down." );
	}
	...
	protected static class MaintThread extends Thread {
		...
		private ConsistentHashing pool;
		private long interval      = 1000 * 3; // every 3 seconds
		private boolean stopThread = false;
		private boolean running;

		protected MaintThread( ConsistentHashing pool ) {
			this.pool = pool;
			this.setDaemon( true );
		}
	...
	public static class SockIO {
		 ...
		 */
		public SockIO( ConsistentHashing pool, String host, int port, int timeout, int connectTimeout, boolean noDelay ) throws IOException, UnknownHostException {
		 ...
		 * @throws UnknownHostException if hostname is invalid
		 */
		public SockIO( ConsistentHashing pool, String host, int timeout, int connectTimeout, boolean noDelay ) throws IOException, UnknownHostException {
