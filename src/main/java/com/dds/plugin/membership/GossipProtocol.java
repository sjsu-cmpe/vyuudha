...
public class GossipProtocol implements NotificationListener, MembershipInterface{
	...
	private int t_gossip = 1200; //in ms
	...
	public void start() {
		...
		//t_gossip = 5000; // 500 for 5 seconds
		random = new Random();
	...
	public Node getMemberToNotify() {
		...
		if(GlobalVariables.INSTANCE.nodeList.size() > 1) {
			...
			if (random.nextInt() % 3 == 0) {
				System.out.println("I am not alone in this world");
			}
		}
		else {
			if (random.nextInt() % 3 == 0) {
						...
						if(GlobalVariables.INSTANCE.containsLiveNode(remoteMember.getNodeId())) {
							...
							if(GlobalVariables.INSTANCE.containsDeadNode(remoteMember.getNodeId())) {
								...
								if(remoteMember.getHeartbeat() > localDeadMember.getHeartbeat()) {
									...
									GlobalVariables.INSTANCE.addToLiveNode(newLocalMember);
